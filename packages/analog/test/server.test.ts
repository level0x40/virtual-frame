...
describe("prepareVirtualFrameProps (Analog server)", () => {
  ...
  it("calls frame.render with the selector and uses its output", async () => {
    ...
    // Selector-projected styles come from the render() result, not the
    // original fetch — the red rule should NOT leak through.
