  ...
  template: `
    <div class="page">
      ...
      <p class="subtitle">
        ...
        <strong>remote</strong> (port 3011) during SSR via <code>fetchVirtualFrame()</code>, then
        VirtualFrame mirrors on the client via a hidden iframe.
      </p>

      <ng-container *ngIf="data">
        <div class="panel info">
          <strong>How it works:</strong> The host calls <code>fetchVirtualFrame()</code> in a Nitro
          server route to fetch the remote Analog page during SSR.
          <code>prepareVirtualFrameProps()</code> wraps the content in declarative shadow DOM for
          instant display. On the client, a hidden <code>&lt;iframe src&gt;</code> loads the remote app
          at its real origin — the <strong>cross-origin bridge</strong> handles live DOM mirroring via
          <code>postMessage</code>. Two <code>&lt;virtual-frame&gt;</code> components are rendered — one
          showing the full page, one showing only <code>#counter-card</code>. Both
          <strong>share a single hidden iframe</strong> (ref-counted). The <code>store</code> input
          bridges <code>&#64;virtual-frame/store</code> state between the host and remote via a
